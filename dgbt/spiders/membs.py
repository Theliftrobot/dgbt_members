...
def add_rows_1(row_dict, worksheet, client, acc):
    headers = ["Membro", "Teléfono", "Suscripción_total"]
    new_row = [''] * 3
...
class MembsSpider(scrapy.Spider):
    ...
    def parse(self, response):
        # pgs_loop = response.meta['pghs_loop']
        ...
        # if not pgs_loop:
            ...
            # req= requests.get(membs_url, headers=headers)
            # res_bdy = req.json()
            # print(res_bdy)
    ...
    def parse_list(self, response):
        ...
        for mid in mids:
            ...
            # res_dict =  add_rows(mem_url, headers, self.client, account)

            # yield{
            #     'Membro': res_dict.get('Membro'),
            #     'Teléfono':  res_dict.get('Teléfono'),
            #     'Suscripción_total':  res_dict.get('Suscripción_total'),
            #     'account': account
            # }   
    ...
    def mem_details(self, response):
        ...
        print(worksheet)
        add_rows_1(row_dict, worksheet, self.client, account)
